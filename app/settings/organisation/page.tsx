...
export default function OrganisationPage() {
  ...
  const [mats, setMats] = useState<MATData[]>(initialMATs)
  const [standaloneSchools, setStandaloneSchools] = useState<SchoolData[]>(initialStandaloneSchools)
  const [expandedMATs, setExpandedMATs] = useState<Set<string>>(new Set(["mat-1"]))
  const [selectedItem, setSelectedItem] = useState<{ type: "mat" | "school"; id: string; matId?: string } | null>(null)
  const [editModalOpen, setEditModalOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<MATData | SchoolData | null>(null)
  const [editingType, setEditingType] = useState<"mat" | "school">("mat")
  const [editingMatId, setEditingMatId] = useState<string | null>(null)

  const toggleMATExpanded = (matId: string) => {
  ...
  const handleSelectItem = (type: "mat" | "school", id: string, matId?: string) => {
    setSelectedItem({ type, id, matId })
  }

  const getSelectedData = (): MATData | SchoolData | null => {
    if (!selectedItem) return null
    
    if (selectedItem.type === "mat") {
      return mats.find(m => m.id === selectedItem.id) || null
    } else {
      // Check standalone schools first
      const standalone = standaloneSchools.find(s => s.id === selectedItem.id)
      if (standalone) return standalone
      
      // Check schools within MATs
      if (selectedItem.matId) {
        const mat = mats.find(m => m.id === selectedItem.matId)
        return mat?.schools.find(s => s.id === selectedItem.id) || null
      }
    }
    return null
  }
  ...
  const handleEditClick = () => {
    ...
    if (data && selectedItem) {
      setEditingItem(JSON.parse(JSON.stringify(data)))
      setEditingType(selectedItem.type)
      setEditingMatId(selectedItem.matId || null)
      setEditModalOpen(true)
    }
  }
  ...
  const handleSaveEdit = () => {
    ...
    if (editingType === "mat") {
      ...
      if (editingMatId) {
        ...
        setMats(prev => prev.map(m => 
          m.id === editingMatId 
            ? { ...m, schools: m.schools.map(s => s.id === editingItem.id ? editingItem as SchoolData : s) }
            : m
        ))
      } else {
        // Standalone school
        setStandaloneSchools(prev => prev.map(s => 
          s.id === editingItem.id ? editingItem as SchoolData : s
        ))
      }
    }
    setEditModalOpen(false)
            ...
            <Card className="w-80 flex flex-col">
              ...
              <CardContent className="flex-1 overflow-auto">
                <div className="space-y-1">
                  {/* MATs with their schools */}
                  {mats.map((mat) => (
                    <div key={mat.id}>
                      {/* MAT Item */}
                      <div
                        className={`flex items-center gap-2 px-2 py-2 rounded-md cursor-pointer transition-colors ${
                          selectedItem?.type === "mat" && selectedItem.id === mat.id
                            ? "bg-slate-100"
                            : "hover:bg-slate-50"
                        }`}
                        onClick={() => handleSelectItem("mat", mat.id)}
                      >
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            toggleMATExpanded(mat.id)
                          }}
                      ...
                      {/* Schools under MAT */}
                      {expandedMATs.has(mat.id) && (
                        <div className="ml-6 border-l border-slate-200 pl-2 space-y-1 mt-1">
                          {mat.schools.map((school) => (
                            <div
                              key={school.id}
                              className={`flex items-center gap-2 px-2 py-2 rounded-md cursor-pointer transition-colors ${
                                selectedItem?.type === "school" && selectedItem.id === school.id
                                  ? "bg-slate-100"
                                  : "hover:bg-slate-50"
                              }`}
                              onClick={() => handleSelectItem("school", school.id, mat.id)}
                            >
                              <School className="w-4 h-4 text-slate-500" />
                    ...
                    <>
                      ...
                      {standaloneSchools.map((school) => (
                        <div
                          ...
                          onClick={() => handleSelectItem("school", school.id)}
                        >
                          <School className="w-4 h-4 text-slate-500" />
                          <span className="text-sm text-slate-700 truncate flex-1">
                            {school.name}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-lg">
                      {selectedData ? selectedData.name : "Select an organisation"}
                    </CardTitle>
                  </div>
                  {selectedData && (
                    <Button 
              ...
              <CardContent className="flex-1 overflow-auto">
                {selectedData ? (
                  <div className="space-y-6">
                    ...
                    {/* Parent MAT for school */}
                    {selectedItem?.type === "school" && selectedItem.matId && (
                      <div>
                        <h3 className="text-sm font-semibold text-slate-900 mb-3">Parent Trust</h3>
                        <p className="text-sm text-slate-600">
                          {mats.find(m => m.id === selectedItem.matId)?.name}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="h-full flex items-center justify-center text-slate-400">
                    <div className="text-center">
                      <Settings className="w-12 h-12 mx-auto mb-3 opacity-50" />
                      <p>Select an organisation or school from the tree to view settings</p>
                    </div>
                  </div>
                )}
              </CardContent>
      ...
      <Dialog open={editModalOpen} onOpenChange={setEditModalOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] p-0 flex flex-col">
          <div className="p-6 pb-0">
            <h2 className="text-lg font-semibold text-slate-900">
              Edit {editingType === "mat" ? "Trust" : "School"} – {editingItem?.name}
            </h2>
          </div>
                  ...
                  <div className="space-y-3">
                    {Object.entries(editingItem.features).map(([feature, enabled]) => (
                      <div key={feature} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                        ...
                        <Switch
                          checked={enabled}
                          onCheckedChange={(checked) => handleFeatureToggle(feature as keyof MATData["features"], checked)}
          ...
          <div className="px-6 py-4 border-t flex justify-end gap-3">
            <Button
              variant="outline"
              onClick={() => setEditModalOpen(false)}
              className="px-4 border-slate-300 text-slate-600"
            >
              Cancel
            </Button>
            <Button

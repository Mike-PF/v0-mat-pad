...
import ReasonsHeatmap from "./ReasonsHeatmap";
import PupilGroupHeatmap from "./PupilGroupHeatmap";
...
const AttendanceStaticReport = ({ setIsCustomDashboard, panelData, filterValues, schools, filters, isCustomDashboard }) => {
	...
	const filteredArray = panelData?.data?.panel8?.filter(
		(value, index, self) => index === self.findIndex((t) => t.place === value.place && t.name === value.name),
	);

	const MATChartData = filteredArray?.map((n) => {
		return {
			name: n?.name,
			y: n?.y,
						...
						{panelData?.data?.panel4 && (
							<button
								...
								<div className="flex-col items-center h-14 justify-center">
									...
									<div className="font-semibold text-sm flex items-center justify-center">{`(nat. ${panelData?.data?.panel4?.national?.toFixed(
										2,
									)}%)`}</div>
								</div>
					...
					<div className="relative lg:col-span-1 h-40 p-2 ">
						{panelData?.data?.panel5 && (
							<div className="w-full h-full overflow-hidden border-none bg-white rounded-xl p-2 flex flex-col items-center justify-center">
								...
								{lastWeekPercentage(panelData?.data?.panel5?.pct, panelData?.data?.panel5?.lastWeekPct, true)}
							</div>
						)}
					</div>
					<div className="relative lg:col-span-1 h-40 p-2 ">
						...
						<div className="col-span-3 h-full md:col-span-12 lg:col-span-4 p-2">
							{panelData?.data?.panel8 ? (
								<div className="bg-white flex items-center justify-center h-full min-h-96 rounded-xl p-3">
									<PieChart
										chartData={MATChartData}
										title={"% pupils by attendance band"}
									/>
								</div>
							) : (
								noDataCard()
								...
								<div className="bg-white flex items-center justify-center h-full min-h-96 rounded-xl">
									{_.isArray(panelData?.data?.panel11?.schools) ? (
										<MATAverageChart
											...
											title={`School with absence above ${getAverageName()} average`}
											name={getAverageName()}
											type="a"
										/>
									) : (
							...
							{panelData?.data?.panel12 ? (
								<div className="bg-white flex items-center justify-center h-full min-h-96 rounded-xl">
									<PupilGroupHeatmap
										...
										title="Attendance by pupil group"
									/>
								</div>
							) : (
								noDataCard()
...
export default AttendanceStaticReport;
